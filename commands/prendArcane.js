...
module.exports = {
	...
	execute(message, args) {
		...
		if ( me_only == 0 ){
			message.channel.send({ content: `${message.author}`+": __Tirage d'une Arcane pour "+board.player[my_player_number]+" :__\n"+msg}, { split: true });
		} else {
			message.author.send({ content: `${message.author}`+": __Tirage d'une Arcane pour "+board.player[my_player_number]+" :__\n"+msg}, { split: true });
		}
	},
};
